...
const CreateActivityPage: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (values.full_description) formData.append('full_description', values.full_description);
      formData.append('category', categoryObj.id.toString());
      ...
      formData.append('min_participants', (values.min_participants || 1).toString());
      
      if (values.required_skills && values.required_skills.length > 0) {
        formData.append('required_skills', JSON.stringify(values.required_skills));
      }
      if (values.age_requirement) formData.append('age_requirement', values.age_requirement);
      if (values.physical_requirements) formData.append('physical_requirements', values.physical_requirements);
      if (values.equipment_needed) formData.append('equipment_needed', values.equipment_needed);
      ...
      console.log('Sending activity data with images...');
  ...
  const handleImageListChange: UploadProps['onChange'] = ({ fileList }) => {
  ...
  const beforeUpload = (file: File) => {
    ...
    if (!isImage) {
      message.error(t('activities.imageOnly'));
      return Upload.LIST_IGNORE;
    }
    const isLt5M = file.size / 1024 / 1024 < 5;
    if (!isLt5M) {
      message.error(t('activities.imageSizeLimit'));
      return Upload.LIST_IGNORE;
    }
    return false; // 阻止自动上传
  };
    ...
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <Card>
        ...
        >
          ...
          <Form.Item
            label={t('activities.activityImages')}
            name="images"
          >
            <Upload
            ...
            >
              {imageList.length < 5 && (
                <div>
                  <PlusOutlined />
                  <div style={{ marginTop: 8 }}>{t('activities.uploadImages')}</div>
                </div>
              )}
            </Upload>
          </Form.Item>
