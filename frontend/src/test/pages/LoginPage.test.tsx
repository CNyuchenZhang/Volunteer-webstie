...
describe('LoginPage', () => {
  ...
  it('应该显示必填字段验证错误', async () => {
    ...
    // 查找表单元素 - 尝试多种选择器
    let form = container.querySelector('form[name="login"]') as HTMLFormElement | null;
    if (!form) {
    ...
    if (!form) {
      const submitButtons = screen.getAllByRole('button', { name: /auth.login/i });
    ...
    await waitFor(() => {
      ...
      const errorTexts = screen.queryAllByText(/auth\.(emailRequired|passwordRequired)/);
